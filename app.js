...
//get all replies of a tweet with tweet_id if the user follows the tweeter
...
WHERE 
tweet.user_id = (SELECT user_id FROM user WHERE user_name = "${request.username}"
...
tweet
WHERE tweet_id = ${tweetId}
AND user_id = (SELECT user_id FROM user WHERE user_name = "${request.username}");
`);
